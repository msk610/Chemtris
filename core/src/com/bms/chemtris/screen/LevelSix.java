...
/**
 * Screen that displays level 6 of the game.
 */
public class LevelSix implements Screen {
    //game state members
    ...
    public void show() {
        ...
        camera.position.set(14f, 14f, 14f);
        camera.lookAt(0, 0, 0);
        camera.near = 1f;
        camera.far = 300f;
        camera.update();
        ...
        matrix = new boolean[10][20][10];

        //setup platform
        platform = new Platform(10f,camera,matrix);

        //setup storage
        storage = new Storage(matrix,camera);

        //setup checker
        checker = new Checker(storage,10);

        //setup game controller
        controller = new Controller(storage,15f,platform.box);

        //setup spawner
        spawner = new Spawner(6,matrix,camera);
    ...
    public void render(float delta) {
        ...
        switch (gameState) {
            ...
            case CHECK:
                ...
                score = points;
                if(score > ScoreManager.getScore(ScoreManager.LEVEL_6)){
                    ScoreManager.setScore(ScoreManager.LEVEL_6,score);
